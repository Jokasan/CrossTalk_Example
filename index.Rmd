---
author: "Nils Indreiten"
output: html_document
---

```{r setup, include=FALSE}
# Read in the tidy tuesday week 35 data
lemurs <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2021/2021-08-24/lemur_data.csv')
pacman::p_load(tidyverse, reactable,plotly,crosstalk,htmltools)
```

```{r, echo=FALSE}
set.seed(123)

small_df <- lemurs %>% 
  select(weight_g,age_at_wt_y, birth_type) %>% 
  sample_n(1000) %>% 
  na.omit()

pen_df <- SharedData$new(small_df)

x <- list(title="Age when weight taken (y)", range= c(0,40))
y <- list(title="Weight (g)", range=c(0,7000))

plotly_graphic <- plot_ly(pen_df,
                          x = ~age_at_wt_y,
                          y = ~weight_g,
                          color = ~birth_type,
                          text = ~birth_type) %>% 
  add_markers() %>% 
  layout(
    xaxis=~x,
    yaxis=~y
    ) %>% 
  highlight(
    on = "plotly_selected",
    off = "plotly_doubleclick",
    persistent = FALSE
      ) %>% 
  config(displaylogo=FALSE)

reactable_table <-  reactable(
  pen_df,
  columns = list(
    birth_type = colDef("Birth Type"),
    weight_g = colDef("Weight"),
    age_at_wt_y = colDef("Age"))
  )

```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
div(
  h1("Lemurs data"),
  h3("Filter by: Birth Type or Weight"),
  br(),
  bscols(
    widths = c(2, 10),
    list(
      filter_checkbox("birth_type", "Birth Type", pen_df, ~birth_type),
      filter_slider("weight_g", "Weight (g)", pen_df, ~weight_g)
    ),
    plotly_graphic,
  br(),
  reactable_table
  )
)  
```

> Birth type refers to whether the animal was captive-born (CB), wild-born (WB) or of unknown birth type (UNK). Data is from [Tidy Tuesday week 35.](https://github.com/rfordatascience/tidytuesday/blob/master/data/2021/2021-08-24/readme.md)
